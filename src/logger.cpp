...
// region ################################################ printer line
void esp_log_print_lines(int lvl, String pLine, const char *func, const char *var, const char *tag ){
    ...
    if (pLine != "") {
        if (pTag != "")     {
            ...
            if (searchCR > 0)   {
                ...
                if (varP != "") {
                    fssprintf(buffer, "[%-70s][%-20s]%s%s\t[%s]", 
                            pLine.c_str(), 
                            varP.c_str(),
                            title.c_str(), 
                            cr.c_str(),
            ...
            if (varP != "") {fssprintf(buffer, "[%-70s][%-20s]%s", pLine.c_str(), varP.c_str(), title.c_str()); }
            else {fssprintf(buffer, "[%-70s]%s", pLine.c_str(), title.c_str()); }
        }       
        ...
        if (pTag != "")     {
            ...
            if (searchCR > 0)   {
                ...
                if (varP != "") {
                    fssprintf(buffer, "[%-20s]%s[%s]", 
                            varP.c_str(),
                            title.c_str(), 
                            pTag.c_str()
                        ); 
                } else {
                    fssprintf(buffer, "%s[%s]", 
                            title.c_str(), 
                            pTag.c_str()
                        ); 
                }
            ...
            if (varP != "") {fssprintf(buffer, "[%-20s]%s", varP.c_str(), title.c_str()); }
            else {fssprintf(buffer, "%s",  title.c_str()); }
        } 
    }

    _looger->printer_serial(buffer);

}
// endregion >>>> printer line

void adriTools_logger::printer_serial(char * buffer){
    int heap            = ESP.getFreeHeap();
    String currenttime  = timeStamp();
    char bufferPrint[255];
    if ((printer_timestamp) && (printer_heap)) {fssprintf(bufferPrint, "[%-8s][%-8d]%s", currenttime.c_str(), heap, buffer);}
    else {
        if (printer_timestamp)  fssprintf(bufferPrint, "[%-8s]%s", currenttime.c_str(), buffer);
        if (printer_heap)       fssprintf(bufferPrint, "[%-8d]%s", heap, buffer);
    } 
    if (spiff_sav) spiff_addLine(String(bufferPrint));
    fsprintf("%s\n", bufferPrint);
}

void  esp_log_printf_(int actif, int lvl, int mod, const char *line, const char *func, const char *var, const char *tag) {
    ...
    switch (mod) {
        case 1:
            ...
            break;
        case 2:
            pLine = String(line) + " " + String(func);       
            break;
        default:
            break;
    }
    esp_log_print_lines(lvl, pLine, func, var, tag );
}
void  esp_log_printf_(int actif, int lvl, int mod, const char *line, const char *func, const char *tag) {
    if (!adriTools_logger_ptrGet()->printer_displayMod(actif, lvl)) return;
    String pLine = "";
    switch (mod) {
        case 1:
    ...
    esp_log_print_lines(lvl, pLine, func, (char*)"", tag );
} 
